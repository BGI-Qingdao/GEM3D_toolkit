...
Usage :
    GEM_toolkit.py apply_alignment -i <input.json or input.csv>
                       ...
                       -a [F/B, forward or backward affine matrix, default F]
                       -m [True/False, merge all h5ad to one if True, default False]
                       -S [True/False, add Sn to cell name if -m=True, default True]
...
#     6. the 3D alignment affine matrix, required.
#     7. the z value for this data, required.
#     8. the 2D registration affine matrix, optional, if you align 3D by raw ssDNA, please provide this.
...
def apply_alignment_main(argv:[]):
    ...
    Sflag=True
    try:
        opts ,args =getopt.getopt(argv,"hi:o:a:m:t:W:H:S:",["help=",
                                                     "input=",
                                                     "output=",])
    ...
    N=len(collections)
    for i in range(N):
        if collections[5] == '' or collections[6] == '' :
            print('Error: 3D affine matrix not exist! exit ...',flush=True)
            sys.exit(1)
        try:
            if collections[7] != '' :
                if a == 'F':
                    affine_2D_b=np.matrix(np.array(json.loads(collections[7]))).T
                    affine_3D_f=np.matrix(np.array(json.loads(collection[5])))
                elif a == "B":
                    affine_2D_b=np.matrix(np.array(json.loads(collections[7])))
                    affine_3D_f=np.matrix(np.array(json.loads(collection[5]))).I
            ...
            sys.exit(1)
        if collection[1]!='': #gem
            affine_gem(collection[1],prefix,affine,collection[0],collection[6])
        if collection[2]!='': #h4ad
            h5ad=affine_h5ad(collection[2],prefix,affine,collection[0],collection[6],Sflag)
            if hflag==False:
...
def affine_h5ad(inputh5ad,prefix,affine,Sn,zvalue,xmin,ymin,Sflag):
